/**
 * X.com List Settings Modal
 * 
 * Modal for editing timeline settings for an X.com list.
 */

import { useState, useEffect } from 'react';
import { X, Settings, Palette, Maximize2, Hash } from 'lucide-react';
import type { XcomList, XcomTimelineSettings } from '../../types/xcom';
import { validateTimelineSettings } from '../../utils/xcomEmbed';

interface XcomListSettingsModalProps {
  list: XcomList;
  onSave: (settings: XcomTimelineSettings) => Promise<void>;
  onClose: () => void;
}

export function XcomListSettingsModal({
  list,
  onSave,
  onClose,
}: XcomListSettingsModalProps) {
  const [settings, setSettings] = useState<XcomTimelineSettings>(list.settings || {});
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-400 mt-1">
                  @{list.ownerScreenName}/lists/{list.slug} {list.displayName && `(${list.displayName})`}
                </p>
              </div>
            </div>
            <button
