...
| POST | `/api/claims/accept` | Session | Idempotent accept onto claim membership; body `{ claimId: string }` → `{ ok, acceptedClaimIds }`. Also tracks the claim by default (Track-on-Accept, idempotent). `400` on missing/empty `claimId`; `404` when claim not found. |
...
Public Kite brief routes under `/api/batches…` are **in addition** to this surface (NEWS-44); they are not a replacement for `/api/articles`. Brief **content** is accepted-only after [NEWS-65](https://informedcrew.atlassian.net/browse/NEWS-65), minus **global mute** ([NEWS-60](https://informedcrew.atlassian.net/browse/NEWS-60)); story membership routes above gate what appears on `/`. **Claim** Accept/Track ([NEWS-75](https://informedcrew.atlassian.net/browse/NEWS-75)) updates claim membership/tracked stores on `/radar`; Brief hybrid rendering of accepted claims is [NEWS-76](https://informedcrew.atlassian.net/browse/NEWS-76) — not yet on `/`. Manual seeds are Accepted on create and excluded from Radar. Track / Untrack are independent of Accept / Unaccept for both stories ([NEWS-59](https://informedcrew.atlassian.net/browse/NEWS-59)) and claims ([NEWS-75](https://informedcrew.atlassian.net/browse/NEWS-75)); Accept and seed track by default. **Mute is shared:** one `mute-rules.json` store and `/api/brief/mutes` CRUD filter story headline clusters **and** claim inbox items (`claimMatchesMute`); no per-claimId mute API in v1. Mute hides clusters/claims on Radar and vetoes Brief eligibility without auto-untrack; tracked muted story clusters still appear on `GET /api/brief/tracked` with `muted: true`. After `POST /api/fetch`, tracked story entries may get `pendingUpdate: true` when member count grows ([NEWS-61](https://informedcrew.atlassian.net/browse/NEWS-61)); tracked claim entries get `pendingUpdate: true` when new evidence links or stance changes arrive. Footer badge sums pending updates across both `GET /api/claims/tracked` and `GET /api/brief/tracked`.
...
**Brief hybrid ([NEWS-76](https://informedcrew.atlassian.net/browse/NEWS-76)):** Accepted claims on `/` alongside linked story clusters — not frozen here until shipped.

**Store-only ([NEWS-70](https://informedcrew.atlassian.net/browse/NEWS-70)):** Runtime data files under `mvp/data/`: `claims.json`, `evidence-links.json`, `claim-membership.json`, `tracked-claims.json`, `claim-review-queue.json`. Claim accept/track HTTP ships in [NEWS-75](https://informedcrew.atlassian.net/browse/NEWS-75); mute reuses shared `mute-rules.json` (no per-claimId mute store).
